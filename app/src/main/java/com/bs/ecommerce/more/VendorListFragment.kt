...
import com.bs.ecommerce.more.adapter.VendorAdapter
...
class VendorListFragment : BaseFragment() {
    ...
    private fun initView() {
            ...
            override fun onClick(view: View, position: Int, data: ProductByVendorData) {
                ...
                when (view.id) {
                    ...
                    R.id.btnContactVendor ->
                        if (data.id != null)
                            replaceFragmentSafely(ContactVendorFragment.newInstance(data.id, data.name))
        ...
        rvList?.apply {
            ...
            listAdapter = VendorAdapter(clickListener)
            adapter = listAdapter
        }
